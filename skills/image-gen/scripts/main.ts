...
Env file load order: CLI args > process.env > <cwd>/.claude-skills/.env > ~/.claude-skills/.env`);
}
...
async function loadEnv(): Promise<void> {
  ...
  const homeEnv = await loadEnvFile(path.join(home, ".claude-skills", ".env"));
  const cwdEnv = await loadEnvFile(path.join(cwd, ".claude-skills", ".env"));
