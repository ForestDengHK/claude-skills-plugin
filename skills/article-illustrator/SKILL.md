...
```bash
test -f .claude-skills/article-illustrator/EXTEND.md && echo "project"
test -f "$HOME/.claude-skills/article-illustrator/EXTEND.md" && echo "user"
```
...
**Filename**: 01-infographic-concept-name.webp
...
2. After each generation, convert to WebP for web optimization:
   ...
   # For scenes/abstract without text - standard quality
   npx -y sharp-cli -i input.png -o output.webp -f webp -q 85 -- resize 1200
...
```markdown
![description](illustrations/{slug}/NN-{type}-{slug}.webp)
```
...
Positions:
- 01-xxx.webp → After "[Section]"
- 02-yyy.webp → After "[Section]"
